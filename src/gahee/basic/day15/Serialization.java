...
public class Serialization {
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        // 직렬화 , Serialization
        // 문자 단위 파일 입출력 시 저장된 데이터들은 모두 문자처럼 다뤄짐.
        // 문자: 혜교, 숫자: 87 => (파일로 저장) => 문자: 혜교, 문자: 87

        // number.txt에 저장된 정수를 읽어 더한 후 출력
        ...
        System.out.println(nums);                       // (1) 파일 읽기
        ...
        // 형변환을 통해 숫자로 바꾼 후 모두 더함

        System.out.println(sum);                        // (2) 형변환 후 계산
        
        br.close();
        fr.close();

        // 반면, 데이터의 고유속성을 그대로 유지하면서
        // 파일형태로 저장하고 싶은 경우, 직렬화를 이용해야함.
